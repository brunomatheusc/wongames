...
describe('Explore Page', () => {
	...
	it('should render filters columns', () => {
		...
		[priceFields, platformFields, sortFields, genresFields].map(field => cy.getFields(field));
	});
	...
	it('should order by price', () => {
		...
		cy.getByDataCy('game-card').first().within(() => {
			cy.shouldBeLessOrGreater(0, 'greater');
		});
	});
	...
	it('should return empty when no games matched', () => {
		cy.visit('/games');
